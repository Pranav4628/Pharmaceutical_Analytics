...
function DemandForecast() {
  ...
  return (
    <div className="card card-p anim-fade-up" style={{ marginBottom:18 }}>
      <SectionHdr title="AI Demand Forecasting" sub="6-month ML prediction using Gradient Boosting (R² = 0.847)" icon={Brain} color="var(--violet-hi)" />
        ...
        <div style={{ marginTop:22 }}>
          <div style={{ display:'flex', gap:20, flexWrap:'wrap', marginBottom:16, padding:'12px 16px', background:'rgba(124,58,237,0.08)', border:'1px solid rgba(124,58,237,0.2)', borderRadius:10 }}>
            {[
              ...
              {l:'Model R²',v:result.model_r2,c:'#34d399'},
  ...
  return (
    <div className="anim-fade-up">
      ...
      <div style={{ display:'flex', gap:12, marginBottom:22, flexWrap:'wrap' }}>
        {[
          {l:'Demand Model',v:'GBM · R²=0.847',c:'var(--violet-hi)'},
          {l:'Revenue Model',v:'GBM · R²=0.921',c:'var(--cyan)'},
          {l:'Anomaly Model',v:'Isolation Forest · 5% contamination',c:'var(--amber-hi)'},
          {l:'Training Data',v:'200,000 records',c:'#34d399'},
        ].map((s,i) => (
          <div key={i} style={{ padding:'8px 16px', background:'rgba(124,58,237,0.08)', border:'1px solid rgba(124,58,237,0.2)', borderRadius:9, display:'flex', gap:8, alignItems:'center' }}>
            <span className="badge badge-ai">{s.l}</span>
            <span style={{ fontSize:12.5, color:s.c, fontWeight:600 }}>{s.v}</span>
          </div>
        ))}
      </div>
