...
function RegionRadarTip({ active, payload, label }) {
  if (!active || !payload?.length) return null;
  const row = payload?.[0]?.payload;
  if (!row) return null;
  return (
    <div style={{ background: 'var(--bg-card2)', border: '1px solid var(--border-hi)', borderRadius: 10, padding: '10px 14px', fontSize: 12 }}>
      <p style={{ color: 'var(--text-secondary)', marginBottom: 7, fontWeight: 600 }}>{label || row.region}</p>
      <p style={{ color: '#00f5d4', fontWeight: 700, marginBottom: 2 }}>Revenue: {fmt.inrAuto(row.revenue)}</p>
      <p style={{ color: '#7c3aed', fontWeight: 700, marginBottom: 2 }}>Qty: {fmt.k(row.qty)}</p>
      <p style={{ color: '#10b981', fontWeight: 700, marginBottom: 2 }}>Profit: {fmt.inrAuto(row.profit)}</p>
    </div>
  );
}
...
export default function Analytics() {
  ...
  const radarRegions = regions.slice(0, 6);
  const maxRevenue = Math.max(1, ...radarRegions.map(r => Number(r.revenue || 0)));
  const maxQty = Math.max(1, ...radarRegions.map(r => Number(r.qty || 0)));
  const maxProfit = Math.max(1, ...radarRegions.map(r => Number(r.profit || 0)));
  const radarData = radarRegions.map(r => {
    ...
    return {
      ...
      qty,
      profit,
      revenueScore: Math.round((revenue / maxRevenue) * 100),
      qtyScore: Math.round((qty / maxQty) * 100),
      profitScore: Math.round((profit / maxProfit) * 100),
    };
  });
            ...
            <div className="chart-card">
              ...
              <ResponsiveContainer width="100%" height={220}>
                <RadarChart data={radarData} cx="50%" cy="50%" outerRadius="72%">
                  ...
                  <Radar name="Revenue" dataKey="revenueScore" stroke="#00f5d4" fill="#00f5d4" fillOpacity={0.14} strokeWidth={2} />
                  <Radar name="Qty" dataKey="qtyScore" stroke="#7c3aed" fill="#7c3aed" fillOpacity={0.12} strokeWidth={2} />
                  <Radar name="Profit" dataKey="profitScore" stroke="#10b981" fill="#10b981" fillOpacity={0.10} strokeWidth={2} />
                  <RT content={<RegionRadarTip />} />
                </RadarChart>
              </ResponsiveContainer>
            </div>
          </div>
